...
class AppManager:
    def __init__(self):
        self.root = tk.Tk()
        # [THIẾT KẾ]: Tiêu đề xuất hiện trên cùng thanh cửa sổ chính
        self.root.title("HỆ THỐNG QUẢN LÝ GUNDAM STORE")

        # [THIẾT KẾ]: Tự động mở rộng giao diện tràn màn hình chính
        try:
    ...
    def setup_style(self):
        """[THIẾT KẾ]: Sắp xếp màu sắc, cỡ chữ và độ cao mặc định của bảng"""
        style = ttk.Style()
        ...
        # Định dạng màu nền khi nhấp chuột chọn một dòng trên bảng
        style.map("Treeview", background=[('selected', '#1e3799')], foreground=[('selected', 'white')])

        # Cấu hình thanh đổi Tab
        style.configure("TNotebook.Tab", padding=[20, 8], font=("Segoe UI", 9, "bold"))

    def clear_current_page(self):
        """Xóa sạch các thành phần giao diện cũ khi chuyển trang"""
        if self.current_page:
            for widget in self.root.winfo_children():
                widget.destroy()

    # Bộ định hướng chuyển giao diện giữa các tệp tin chức năng
