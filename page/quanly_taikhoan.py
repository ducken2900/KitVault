...
class QuanLyTaiKhoanPage:
    ...
    def open_user_popup(self, edit_mode=False):
        ...
        if edit_mode:
            ...
            if not sel:
                ...
                return
            if len(sel) > 1:
                messagebox.showwarning("Thông báo", "Bạn chỉ được phép chọn duy nhất 1 tài khoản nhân viên để chỉnh sửa!")
                return
        ...
        tk.Label(pop, text=title_text, font=("Segoe UI", 14, "bold"), bg="#22242b", fg=self.color_teal).pack(pady=(20, 15))
        ...
        for i, (label_text, key, default_val, editable) in enumerate(fields):
            tk.Label(form, text=label_text, font=("Segoe UI", 9, "bold"), bg="#22242b", fg="#a4b0be").grid(row=i, column=0, sticky="w", pady=8)
            ...
            if key == "Role":
                ...
                e = tk.Entry(form, font=("Segoe UI", 10), bg=self.color_dark, fg="white", insertbackground="#1abc9c",
                             selectbackground=self.color_dark, selectforeground="white", exportselection=0, bd=1, relief="solid", width=12)
        ...
        def save_changes():
            ...
            try:
                if edit_mode:
                    ...
                    if exists:
                        messagebox.showerror("Lỗi", f"Tên đăng nhập '{u}' đã tồn tại! Vui lòng chọn tên đăng nhập khác.")
                        return
    ...
    def export_users_csv(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            title="Chọn nơi lưu danh sách nhân viên CSV"
        )
        if not file_path:
            return
        try:
            rows = self.app_manager.db.query("SELECT username, fullname, phone, role FROM users")
            with open(file_path, "w", encoding="utf-8-sig", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["TÊN ĐĂNG NHẬP", "HỌ VÀ TÊN", "SỐ ĐIỆN THOẠI", "VAI TRÒ"])
                for r in rows:
                    writer.writerow([r[0], r[1], r[2], r[3]])
            messagebox.showinfo("Thành công", "Đã xuất danh sách nhân viên ra file CSV thành công!")
        except Exception as e:
